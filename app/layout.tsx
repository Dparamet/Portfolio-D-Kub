  ...
  return (
    <html lang="en" className="scroll-smooth">
      <body className={`${inter.className} bg-white text-gray-800`}>
        ...
        {/* Floating vertical nav — fixed middle-left */}
          ...
          {navItems.map((item, i) => (
            <a
              ...
              className={`
                ...
                hover:scale-110 hover:shadow-violet-200
                ${i === 0
                  ? "bg-violet-500 text-white"
                  : "bg-white/90 backdrop-blur-sm border border-violet-100 text-gray-500 hover:bg-violet-500 hover:text-white hover:border-violet-500"
                }
              `}
            >
              {item.icon}
              {/* Tooltip */}
              <span className="pointer-events-none absolute left-14 whitespace-nowrap rounded-lg bg-gray-800 px-2.5 py-1 text-xs text-white opacity-0 group-hover:opacity-100 transition-opacity duration-150">
                {item.label}
              </span>
            </a>
          ))}
